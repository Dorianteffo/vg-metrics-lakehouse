...
def clean_games_table(games_df : DataFrame) -> DataFrame :
    ...
    games_df = games_df.withColumn("Rating", F.when(F.col("Rating")=="E", "Everyone") \
...
def clean_activities_table(activities_df : DataFrame) -> DataFrame:
    ...
    activities_df = activities_df.withColumn("Level", F.when(F.col("Level")<30, "Beginner") \
                            .when((F.col("Level")>30) & (F.col("Level")<60), "Mid-Level") \
                            .when(F.col("Level")>60, "Advanced") \
                            .otherwise("Unknown"))       

    activities_df = activities_df.withColumn('StartTime', F.col('StartTime').cast('timestamp'))
    activities_df = activities_df.withColumn('EndTime', F.col('EndTime').cast('timestamp'))


    activities_df = activities_df.withColumn('SessionDuration', (F.col('EndTime').cast('long') - F.col('StartTime').cast('long'))/60)
